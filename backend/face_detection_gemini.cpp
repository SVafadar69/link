...
cv::Mat alignFace(const cv::Mat& img, const Face& face) {
    cv::Point2f left_eye = face.landmarks[0];
    cv::Point2f right_eye = face.landmarks[1];
    double dy = right_eye.y - left_eye.y;
    double dx = right_eye.x - left_eye.x;
    double angle = std::atan2(dy, dx) * 180.0 / CV_PI;

    cv::Point2f eyes_center((left_eye.x + right_eye.x) / 2.0f, (left_eye.y + right_eye.y) / 2.0f)
    cv::Mat rot_mat = cv::getRotationMatrix2D(eyes_center, angle, 1.0);
    cv::Mat aligned_img; 
    cv::warpAffine(img, aligned_img, rot_img, img.size(), cv::INTER_LINEAR, cv::BORDER_CONSTANT, cv::Scalar(0, 0, 0));
    cv::Rect crop_rect(std::max(0, static_cast<int>(face.bbox.x)), std::max(0, static_cast<int>(face.bbox.y)), 
    std::min(aligned_img.cols - static_cast<int>(face.bbox.x), static_cast<int>(face.bbox.width)), 
    std::min(aligned_img.rows - static_cast<int>(face.bbox.y), static_cast<int>(face.bbox.height)));
    return aligned_img(crop_rect);
}
